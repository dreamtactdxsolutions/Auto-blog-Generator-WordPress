...
【重複回避ルール (超重要・厳守)】
以下のタイトルは既にブログ内に存在します。今回の記事がこれらと内容やテーマ、切り口において絶対に重複（かぶり）しないようにしてください：
{existing_titles_list_str}

※特に、上記の既存タイトルに含まれる特定の「店舗名」「ホテル名」「スポット名」（例：Yard miyakojima、キャノピーbyヒルトンなど）を今回の新規記事のメインテーマ・主役にすることは厳禁です。完全に異なる新しい観光地やテーマ、切り口を選定してください。
"""
...
"""
            ...
            exclude_info = f"\n【除外リスト (以下のスポットやテーマは今回の執筆テーマから完全に除外してください)】\n" + "\n".join([f"- {t}" for t in existing_titles]) if existing_titles else ""
            prompt_topic = f"""
【今回の指令 (最新トレンドリサーチ)】
Google検索ツールを使って、2026年現在の宮古島に関する最新の観光情報や流行の話題をくまなくリサーチしてください。
{genre_str}
例えば、以下のようなトピックを調べてリサーチのヒントにしてください：
1. 最近（特に2025〜2026年）オープンした新しいカフェ、レストラン、新名所、商業施設、または新しく開業したリゾートホテルなど（※ジャンルに合致するものをGoogle検索から見つけ出してテーマに選定してください）
2. 近々開催される予定の人気イベントや季節のフェスティバル（例：MIYAKO ISLAND ROCK FESTIVALや地域の夏祭り、伝統行事など）
3. 2026年の旅行客に今最もホットな話題や、トレンド of 楽しみ方・過ごし方
{exclude_info}
...
【執筆ボリュームに関する最重要指示（文字数不足の解決）】
あなたは読者やSEOの観点から最高評価を得るため、非常に詳細で網羅的、かつボリューミーな記事（総文字数3,000文字〜4,000文字程度）を執筆する必要があります。
各見出し（h2, h3, h4など）の本文は、単なる概要や簡潔な紹介にとどめず、現地の詳しい状況、レンタカーでの所要時間、注意点、プロならではのアドバイスなどを含め、段落ごとに最低でも300〜500文字程度のしっかりとしたボリュームで具体的に肉付けして書き下してください。
箇条書き（ul, li）だけで済ませず、説明のための文章をたっぷりと執筆してください。情報の省略や簡潔なまとめは絶対に禁止します。"""
...
【執筆ボリュームに関する最重要指示（文字数不足の解決）】
リライト（改善）にあたっては、狙うクエリに完璧に答えるため、既存の記述をただ並べるだけでなく、圧倒的な情報量と網羅性をもって大幅に文章を加筆してください。
各見出し（h2, h3, h4など）の本文は、段落ごとに最低でも300〜500文字程度の具体的で読み応えのある情報で肉付けし、読者が他のページを検索し直さなくても済むように徹底的に掘り下げて執筆してください。情報の省略や簡潔なまとめは絶対に禁止します。"""
